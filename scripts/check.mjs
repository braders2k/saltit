...
try {
  ...
  const served = (ld.areaServed || []).map((place) => place.name);
  const wantServed = ["Saltdean", "Rottingdean", "Peacehaven", "Woodingdean"];
  const hours = ld.openingHoursSpecification?.[0];
  if (ld.name !== "Salt IT" || ld.email !== "hello@saltit.co.uk" || ld.priceRange !== "££") {
    errors.push("JSON-LD name, email or priceRange is wrong");
  }
  if (served.join("|") !== wantServed.join("|")) errors.push(`JSON-LD areaServed "${served.join(", ")}" is wrong`);
  if (!hours || hours.opens !== "09:00" || hours.closes !== "18:00" || (hours.dayOfWeek || []).join() !== "Monday,Tuesday,Wednesday,Thursday,Friday") {
    errors.push("JSON-LD opening hours are wrong");
  }
  if (!ld.description) errors.push("JSON-LD description missing");
